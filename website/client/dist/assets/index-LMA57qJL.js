...
`);let ue=new ni(new si);new ri(ue);class oi{constructor(e){this.pathname=e}load(){return b.load(async()=>{let e=this.pathname;(e==""||e.endsWith("/"))&&(e+="index");const t=await fetch(`/content/${e}.page`);if(!t.ok)throw new Error(`Response status: ${t.status} - ${t.statusText}`);this.processMarkdown(await t.text())})}processMarkdown(e){this.frontmatter={},e=e.replace(/\r\n/g,`
...
`);ue.register({pattern:"/:pathname*",match:async r=>{try{return r.document=new oi(r.match.groups.pathname),await r.document.load(),r.page=new Lt(r.document),!0}catch{r.page=new it}return!0},order:10});var B,pe;class Ct extends k{constructor(){super();$(this,B);$(this,pe,"");this.init(),require(["vs/editor/editor.main"],()=>{f(this,B,monaco.editor.create(this.editorContainer,{value:a(this,pe),language:"javascript",theme:"vs-dark"})),f(this,pe,null),a(this,B).getModel().onDidChangeContent(i=>{this.dispatchEvent(new Event("input"))}),this.resizeEditor()}),new ResizeObserver(()=>{this.resizeEditor()}).observe(this.editorContainer)}resizeEditor(){a(this,B)&&a(this,B).layout()}get editor(){return a(this,B)}get value(){return a(this,B)?a(this,B).getValue():a(this,pe)}set value(t){a(this,B)?a(this,B).setValue(t):f(this,pe,t)}}B=new WeakMap,pe=new WeakMap,C(Ct,"template",{type:"div",class:"editorContainer",style:"width: 100%; height: 100%;",bind:"editorContainer"});let ye=`<html>
<head>
...
`,pt=!1;function hi(){if(pt)return;pt=!0;let r=Array.from(document.querySelectorAll("link[rel=stylesheet]")).map(e=>`<link href="${e.getAttribute("href")}" rel="stylesheet">`).join(`
`);ye=ye.replace("##stylesheets##",r),ye=ye.replace("##patchlinecount##",(ye.split(`
`).length-1).toString())}class Tt extends k{constructor(e){hi(),super(),this.script=e}get srcdoc(){return`${ye}${this.script}
        new Main().mount("body");
        ${di}
...
`);ue.register({pattern:"/sandbox",match:r=>(r.page=new Dt,!0)});var te;class Pt extends k{constructor(){super();$(this,te,null);ue.addEventListener("didEnter",(t,i)=>{var n;i.page&&(i.page.layout?(i.page.layout!=((n=a(this,te))==null?void 0:n.constructor)&&(f(this,te,new i.page.layout),this.layoutSlot.content=a(this,te)),a(this,te).loadRoute(i)):(this.layoutSlot.content=i.page,f(this,te,null)))})}}te=new WeakMap,C(Pt,"template",{type:"div",id:"layoutRoot",$:[$t,{type:"embed-slot",bind:"layoutSlot"}]});H.declare(`
#layoutRoot
{
    padding-top: var(--fixed-header-height);
}
`);function pi(){new Pt().mount("body"),ue.start()}pi();
